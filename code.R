  ...
  for (i in 1:n) {
    ...
    if (type == "kern") {
      ...
      ff <-
        as.formula(paste(
          "Y ~ ",
          paste0("bs(X", 1:nx, ", knots = ", h, ")", collapse = " + ")
        ))
      mod <- lm(data = train, ff)
      X_test_final <- rbind(select(test, -Y), rep(0, nx))
      Y_pred <- predict(mod, newdata = X_test_final)[1]
...
locloss_2_1 <-
  foreach(
    a = h_loc,
    .packages = c('splines', 'locfit', 'dplyr'),
    .verbose = TRUE,
    .combine = rbind
  ) %dopar% {
...
loc_2_1 <-
  locfit(data = df, lgas ~ lp(lprice, degree = 1, bandwidth = h_loc[locmin_2_1]))
power_2_1 <-
  lm(data = df, lgas ~ poly(lprice, degree = h_power[powermin_2_1]))
...
# Predict lgas values
lgas_pred_loc <-
  predict(loc_2_1, newdata = data.frame(lprice = lprice_pred))
lgas_pred_power <-
  predict(power_2_1, newdata = data.frame(lprice = lprice_pred))
lgas_pred_bspline <-
  predict(bspline_2_1, newdata = data.frame(lprice = lprice_pred))
...
legend(
  "bottomright",
  legend = c("Local linear", 'Power Series', 'BSpline'),
...
Y_pred_loc <-
  predict(loc_2_1, se.fit = TRUE, data.frame(lprice = X_new))
Y_pred_power <-
  predict(power_2_1, se.fit = TRUE, data.frame(lprice = X_new))
Y_pred_bspline <-
  predict(bspline_2_1, se.fit = TRUE, data.frame(lprice = X_new))
...
loc_2_3 <-
  locfit(data = df, lgas ~ lp(lprice, lincome, degree = 1, bandwidth = h_loc[locmin_2_3]))
power_2_3 <-
  lm(data = df, lgas ~ poly(lprice, lincome, degree = h_power[powermin_2_3]))
bspline_2_3 <-
  lm(data = df,
     lgas ~ bs(lprice, knots = h_bspline[bsplinemin_2_3]) +  bs(lincome, knots = h_bspline[bsplinemin_2_3]))
...
lasso <-
  rlasso(
    data = df ,
    lgas ~ lprice + age + driver + hhsize + fueltype + urban + prov + year ,
    post = FALSE
...
for (i in 1:sims) {
  ...
  post_lasso_betas[lasso_coefs@i[lasso_coefs@i != 0]] <-
    coef(post_lasso)
  ...
  post_lassoCV <-
    lm(Y ~ 0 + X[, lassoCV_coefs@i[lassoCV_coefs@i != 0]])
  post_lassoCV_betas[lassoCV_coefs@i[lassoCV_coefs@i != 0]] <-
    coef(post_lassoCV)
